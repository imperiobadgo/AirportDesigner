...
public class Handler {
    ...
    public Handler(Main main, UiManager uiManager) {
        this.main = main;
...
//        Log.i(Game.TAG, "Touched!");
        double lastTouchDistance = CommonMethods.getDistance(new PointInt(x, y), lastTouch);
        ...
        if (GameInstance.Settings().buildMode == 1 && !hasSomethingClicked) {
            ...
            if (selectableGameObjects.size() > 0) {
                ...
                for (ClickableGameObject ClObject : selectableGameObjects) {
                    hasSomethingClicked = clickGameObject(ClObject, (int) x, (int) y, true);
...
//                            toast.show();
        ...
        if (!hasSomethingClicked) {
            if (selectableGameObjects.size() > 0) {
                ...
                ClickableGameObject clickedObject = null;
                for (ClickableGameObject ClObject : selectableGameObjects) {
                    hasSomethingClicked = clickGameObject(ClObject, (int) x, (int) y, false);
                    if (hasSomethingClicked) {
                        clickedObject = ClObject;
                        break;
                    }
...
//                uiManager.removeCircleButtons();
                ...
                for (Object tempObject : airportObjects) {
                    hasSomethingClicked = clickGameObject(tempObject, (int) x, (int) y, false);
                ...
                if (GameInstance.Airport().getConnectionsMissing() != null) {
                    for (ConnectionMissing missing : GameInstance.Airport().getConnectionsMissing()) {
                        hasSomethingClicked = clickGameObject(missing.getTarget(), (int) x, (int) y, false);
...
//            Log.i(Game.TAG, ClObject.toString() + " checked!");
            if (ClObject.isColliding(new PointInt(mx, my))) {
                System.out.println(ClObject.toString() + " clicked!");
                if (justCheckCollision) {
                    return true;
                }
                ClObject.clicked(mx, my);
                setSelected(ClObject);
                uiManager.createCircleButtons(ClObject, mx, my);
...
//            PointInt tablePos = mouseToTablePos((int) lastTouchX, (int) lastTouchY + GameInstance.Settings().buildOffset);
            boolean canBuild = BuildingHelper.calcBuildPos((int) lastTouchX + buildOffset.x, (int) lastTouchY + buildOffset.y, firstRoadIntersection, buildIntersection, lines, intersectPoints);
            if (buildRoad != null) {
